...
public:
    ...
    inline void Set_Target_Chassis_Power_Limit_Max(float __Target_Chassis_Power_Limit_Max);
    ...
    inline float Get_Chassis_Power_Max();
...
protected:
    ...
    //轮轴距
    const float MECANUM_L =  (0.415+0.446)/2;//0.92f/2.0f;       //Lx+Ly(前后轮半轴距+左右轮半轴距)
...
inline void Class_Chassis::Set_Target_Chassis_Power_Limit_Max(float __Target_Chassis_Power_Limit_Max)
{
    Chassis_Power_Limit_Max = __Target_Chassis_Power_Limit_Max;
}
...
inline float Class_Chassis::Get_Chassis_Power_Max()
{
    return Chassis_Power_Limit_Max;
}
