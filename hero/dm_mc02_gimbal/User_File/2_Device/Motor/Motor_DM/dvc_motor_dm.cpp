...
void Class_Motor_DM_Normal::Data_Process()
{
    ...
    Struct_Motor_DM_CAN_Rx_Data_Normal *tmp_buffer =
        (Struct_Motor_DM_CAN_Rx_Data_Normal *)CAN_Manage_Object->Rx_Buffer;
    ...
    tmp_omega = ((uint16_t)tmp_buffer->Omega_11_4 << 4) |
                ((uint16_t)tmp_buffer->Omega_3_0_Torque_11_8 >> 4);

    tmp_torque = (((uint16_t)tmp_buffer->Omega_3_0_Torque_11_8 & 0x0f) << 8) |
                 ((uint16_t)tmp_buffer->Torque_7_0);

    Rx_Data.Control_Status =
        static_cast<Enum_Motor_DM_Control_Status_Normal>(tmp_buffer->Control_Status_Enum);

    /*
     * 达妙 MIT 反馈位置本身就是在 [-P_MAX, +P_MAX] 范围内编码的绝对位置。
     * 腿关节不需要在这里做 Total_Round 多圈累计。
     *
     * 原来的 Total_Round 逻辑在上电第一帧时会误判跨圈：
     * Pre_Encoder 初始为 0，如果第一帧 tmp_encoder > 32768，
     * 就会 Total_Round--，导致 Now_Angle 接近 -25rad。
     */
    Rx_Data.Now_Angle = Basic_Math_Int_To_Float((int32_t)tmp_encoder,
                                                0,
                                                (1 << 16) - 1,
                                                -Angle_Max,
                                                Angle_Max);

    Rx_Data.Now_Omega = Basic_Math_Int_To_Float((int32_t)tmp_omega,
                                                0,
                                                (1 << 12) - 1,
                                                -Omega_Max,
                                                Omega_Max);

    Rx_Data.Now_Torque = Basic_Math_Int_To_Float((int32_t)tmp_torque,
                                                 0,
                                                 (1 << 12) - 1,
                                                 -Torque_Max,
                                                 Torque_Max);
    ...
    Rx_Data.Total_Encoder = (int32_t)tmp_encoder - ((1 << 15) - 1);
}
...
void Class_Motor_DM_Normal::Output()
{
    ...
    case (Motor_DM_Control_Method_NORMAL_MIT):
    {
        ...
        tmp_angle = (uint16_t)Basic_Math_Float_To_Int(Control_Angle,
                                                      -Angle_Max,
                                                      Angle_Max,
                                                      0,
                                                      (1 << 16) - 1);

        tmp_omega = (uint16_t)Basic_Math_Float_To_Int(Control_Omega,
                                                      -Omega_Max,
                                                      Omega_Max,
                                                      0,
                                                      (1 << 12) - 1);

        tmp_torque = (uint16_t)Basic_Math_Float_To_Int(Control_Torque,
                                                       -Torque_Max,
                                                       Torque_Max,
                                                       0,
                                                       (1 << 12) - 1);

        tmp_k_p = (uint16_t)Basic_Math_Float_To_Int(K_P,
                                                    0.0f,
                                                    500.0f,
                                                    0,
                                                    (1 << 12) - 1);

        tmp_k_d = (uint16_t)Basic_Math_Float_To_Int(K_D,
                                                    0.0f,
                                                    5.0f,
                                                    0,
                                                    (1 << 12) - 1);
        ...
        tmp_buffer->Control_Angle_Reverse =
            Basic_Math_Endian_Reverse_16(&tmp_angle, nullptr);

        tmp_buffer->Control_Omega_11_4 =
            (uint8_t)(tmp_omega >> 4);

        tmp_buffer->Control_Omega_3_0_K_P_11_8 =
            (uint8_t)(((tmp_omega & 0x0f) << 4) | (tmp_k_p >> 8));

        tmp_buffer->K_P_7_0 =
            (uint8_t)(tmp_k_p & 0xff);

        tmp_buffer->K_D_11_4 =
            (uint8_t)(tmp_k_d >> 4);

        tmp_buffer->K_D_3_0_Control_Torque_11_8 =
            (uint8_t)(((tmp_k_d & 0x0f) << 4) | (tmp_torque >> 8));

        tmp_buffer->Control_Torque_7_0 =
            (uint8_t)(tmp_torque & 0xff);
